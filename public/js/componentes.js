...
function formularioAvaliacao(){
  return `<form id="formularioAvaliacao" class="formulario" style="padding:14px;border-top:1px solid var(--linha);background:#fff;text-align:center"><strong style="display:block;margin-bottom:8px">Como foi sua experiência?</strong><div class="avaliacao-estrelas">${[5,4,3,2,1].map(i=>`<label title="${i} estrela${i>1?'s':''}"><input type="radio" name="rating" value="${i}" required style="display:none"><span>${'★'.repeat(i)}</span></label>`).join('')}</div><textarea name="comment" placeholder="Comentário opcional..." style="width:100%;margin-top:10px;min-height:50px;border:1px solid var(--linha);border-radius:12px;padding:8px;font-size:var(--fonte-media)"></textarea><button class="btn primary" style="margin-top:8px">Enviar avaliação</button></form>`
}
