...
function comporConversa(t){
  ...
  const podeResponder = ehCliente() || (t.atendente?.id === estado.usuario.id)
  ...
  return `<form id="formularioMensagem" class="chat-compor">
    <div class="barra-formatacao">
      <button type="button" data-formatar="negrito" title="Negrito"><strong>B</strong></button>
      <button type="button" data-formatar="italico" title="Itálico"><em>I</em></button>
      <button type="button" data-formatar="codigo" title="Código">&lt;/&gt;</button>
    </div>
    <div id="previaImagem" class="previa-imagem" style="display:none"></div>
    <textarea id="textoMensagem" name="text" placeholder="Escreva sua resposta... **negrito** *itálico*" rows="1" oninput="this.style.height='';this.style.height=Math.min(this.scrollHeight,160)+'px'"></textarea>
    <div class="chat-compor-acoes">
      <label class="arquivo-inline" title="Anexar imagem">${icone('anexo')}<input name="attachment" type="file" accept="image/png,image/jpeg,image/webp,image/gif" onchange="previaAnexo(this)"></label>
      <button class="btn primary">${icone('enviar')}</button>
    </div>
  </form>`
}
...
function formularioAvaliacao(){
  return `<form id="formularioAvaliacao" class="formulario" style="padding:14px;border-top:1px solid var(--linha);background:#fff;text-align:center"><strong style="display:block;margin-bottom:8px">Como foi sua experiência?</strong><div class="avaliacao-estrelas" id="estrelasAvaliacao"><input type="hidden" name="rating" id="ratingValor" value="0">${[1,2,3,4,5].map(i=>`<button type="button" class="estrela-btn" data-v="${i}" onclick="selecionarEstrela(${i})">★</button>`).join('')}</div><div class="emocao-avaliacao" id="emocaoAvaliacao" style="font-size:32px;margin:4px 0"></div><textarea name="comment" placeholder="Comentário opcional..." style="width:100%;margin-top:10px;min-height:50px;border:1px solid var(--linha);border-radius:12px;padding:8px;font-size:var(--fonte-media)"></textarea><button class="btn primary" style="margin-top:8px">Enviar avaliação</button></form>`
}

function visaoModelos(){
  const modelos = estado.modelos||[]
  return `<div class="grade colunas-2"><div class="painel"><h3>Criar modelo</h3><form id="formularioModelo" class="formulario"><div class="campo"><label>Título</label><input name="title" required placeholder="Ex: Resposta de boas-vindas"></div><div class="campo"><label>Texto</label><textarea name="text" required placeholder="Escreva o texto do modelo..."></textarea></div><button class="btn primary bloco">Salvar modelo</button></form></div><div class="painel"><h3>Modelos salvos</h3>${modelos.length?modelos.map(m=>`<div class="sinalizacao-item"><div><strong>${escaparHtml(m.titulo)}</strong><div style="white-space:pre-wrap;color:var(--opaco);font-size:11px">${escaparHtml(m.texto?.substring(0,120))}${(m.texto||'').length>120?'...':''}</div></div><button class="btn perigo" data-excluir-modelo="${m.id}">Excluir</button></div>`).join(''):'<div class="vazio">Nenhum modelo salvo.</div>'}</div></div>`
}
